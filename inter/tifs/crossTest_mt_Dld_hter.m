...
saveFigDir = [output_dir crossProtcl 'f'];
saveDataDir = [output_dir, crossProtcl 'd'];

if(issave)
    saveas(gcf,saveFigDir,'fig');
    saveas(gcf,saveFigDir,'png');

    save(saveDataDir,'roc_x','roc_y','T','AUC','meanHTER_test', 'stdHTER_test', 'HTER_test', ...
         'testidx', 'trainidx');
end

...
output_dir =  '../../../testpic/motion/inter/256/';
if ~isdir(output_dir)
    mkdir(output_dir);
end
param.nata = 0.1;
...
load('../../../feat_result/3DMAD/motion/conv3_3/y.mat');
y_D3MAD = y;
x_D3MAD = x;
...
if strcmp(crossProtcl, 'ours2D3MAD') %train on ours, test on 3DMAD
    ...
    portion = 6;
        ...
        load('../../../motion/chldis/D3MAD2ours_idx.mat');
...
for i_round = 1:roundNum
    ...
    [ W0, Wi, D ] = mtmultk( new_train_x, train_y, param );
...
if(issave)
    ...
    save(saveDataDir,'roc_x','roc_y','T','AUC','meanHTER_test', 'EER', 'W0_iter', 'Wi_iter', 'D_iter');
end
